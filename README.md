...
5. **🎨 配图与视频**：建议开启配合 aiimg 插件；配图默认复用 aiimg 为人格配置的自拍链路，可填写 `image_chain_override` 手动指定画图 provider 链路（按顺序失败顺延），开启夜间第三套时可用 `image_chain_override_night` 单独指定晚间画图链路；视频生成仅建议为 `greeting` 和 `mood` 开启；智能视频提示词（`enable_smart_video_prompt`）默认开启，可指定独立识图 LLM（`video_llm_provider_id`）。
