...
*   新增 `disable_chat_sharing` 配置项：开启后该人格不会自动分享到任何群聊或私聊（QQ空间分享不受影响），解决"留空默认与全局同配置导致无法让一个人格不分享"的设计缺陷
*   移除全局级人格配置（`llm_conf.persona_id`）：所有人格统一在「多人格配置」列表中管理，全局配置仅作为默认值回退，避免全局级任务无法查找人格级覆盖配置的问题
