...
class ImageService:
    ...
    def _ensure_plugin(self):
        """确保AI图像插件已加载（支持gitee_aiimg和aiimg）"""
...
请输出："""
    ...
    async def _assemble_final_prompt(self, content: str, sharing_type: SharingType, involves_self: bool, visuals: Dict) -> str:
        ...
        if self.debug_mode:
            logger.info("+" * 60)
            logger.info(f"[DailySharing] 【DEBUG】开始组装 Prompt，involves_self={involves_self}")
        ...
        if involves_self:
            ...
            if appearance: 
                ...
                if self.debug_mode:
                    logger.info(f"[DailySharing] 【DEBUG】已加入外貌: {appearance[:50]}...")
            else: 
                prompts.append("1个女孩, 独奏")
                if self.debug_mode:
                    logger.info(f"[DailySharing] 【DEBUG】未找到外貌，使用默认")
            ...
            if raw_outfit: 
                ...
                if self.debug_mode:
                    logger.info(f"[DailySharing] 【DEBUG】已加入 Agent 提取的穿搭: {raw_outfit}")
            else:
                if self.debug_mode:
                    logger.info(f"[DailySharing] 【DEBUG】Agent 未提取到穿搭")
        ...
        if self.debug_mode:
            logger.info(f"[DailySharing] 【DEBUG】组装完成，最终 Prompt 长度: {len(final_result)}")
            logger.info("+" * 60)
    ...
    async def _get_aiimg_reference_images(self) -> List[bytes]:
        ...
        try:
            ...
            logger.warning(f"[DailySharing] 获取AI图像参考图失败: {e}")
    ...
    async def _call_aiimg(self, prompt: str, use_ref_selfie: bool = False) -> Optional[str]:
        ...
        self._ensure_plugin()
        if not self._aiimg_plugin:
            logger.error("[DailySharing] 未找到AI图像插件，请确保已安装 astrbot_plugin_aiimg 或 astrbot_plugin_gitee_aiimg")
            return None

        try:
            # ================= 自拍参考图逻辑 =================
            if use_ref_selfie and hasattr(self._aiimg_plugin, "edit"):
                logger.info("[DailySharing] 尝试使用AI图像插件的自拍参考图生成...")
                
                # 1. 获取参考图
                ref_images = await self._get_aiimg_reference_images()
                if ref_images:
                    logger.info(f"[DailySharing] 找到 {len(ref_images)} 张参考图，调用图生图接口")
                    
                    # 2. 构建 Prompt (支持用户自定义)
                    custom_ref_prompt = self.config.get("selfie_ref_prompt", "").strip()
                    if custom_ref_prompt:
                        try:
                            final_prompt = custom_ref_prompt.format(prompt=prompt)
                        except KeyError as e:
                            logger.warning(f"[DailySharing] 自拍参考图提示词缺少变量: {e}，使用默认模板")
                            final_prompt = self._get_default_selfie_ref_prompt(prompt)
                    else:
                        final_prompt = self._get_default_selfie_ref_prompt(prompt)
                    
                    if self.debug_mode:
                        logger.info("=" * 60)
                        logger.info("[DailySharing] 【DEBUG】自拍模式已激活，参数如下：")
                        logger.info(f"[DailySharing] 【DEBUG】参考图数量: {len(ref_images)}")
                        logger.info(f"[DailySharing] 【DEBUG】Task Types: {['id', 'background', 'style']}")
                        logger.info(f"[DailySharing] 【DEBUG】传入模型的完整 Prompt:\n{final_prompt}")
                        logger.info("=" * 60)
                    
                    # 3. 调用 Edit 接口
                    # 尝试新版调用方式（支持chain_override）
                    try:
    ...
    def _get_default_selfie_ref_prompt(self, prompt: str) -> str:
        """获取默认的自拍参考图提示词"""
        return f"""请根据参考图生成一张新的生活照：
